...
"""
This script  simulates a robot moving on a 5 by 5 units table top with the following commands:

PLACE X,Y,F    --> Placing the robot to the desired location (x,y) and direction (f) 
MOVE           --> Moves the robot 1 unit forward except if it's out of bounds
LEFT           --> Turns the robot to the left of it's current direction without moving it's location
RIGHT          --> Turns the robot to the right of it's current direction without moving it's location
REPORT         --> Shows the current location and direction of the robot (e.g.: 5,0,NORTH)
STOP           --> Command to stop the script gracefully rather than forcing the user to use CTRL+C


There are two ways to use this script
1. Use a text file (e.g.: 'data.txt') with commands inside
2. Use manual mode for flexibility (without using data.txt). The user needs to enter each command (one command at a time).

"""

import sys

available_commands = ["PLACE", "MOVE", "LEFT", "RIGHT", "MOVE", "REPORT", "STOP"]
...
def process_file():
    ...
    """
    filename = str(input("What is the the filename [default: data.txt]?")) or "data.txt" 
    commands = []
    ...
    try:
        ...
        print("\'%s\' file does not exists. Make sure that this file exists." % (filename))
        print(error)

    #check all the commands if valid
    invalid_lines = [commands.index(command) + 1 for command in commands if command.split()[0] not in available_commands]
    if invalid_lines:
        print("Invalid commands found on following line/s of %s: %s" % (filename, invalid_lines))
        sys.exit(0)
...
def place_robot(x, y, face_direction):
    ...
    x,y   --> robot's position in x and y axes. (0,0) in the SOUTHWEST most corner of the table
                where 0 <= x,y <= 5  
    f     --> what direction should the robot face (i.e. NORTH, EAST, SOUTH, or WEST) 
    """
    new_x = int(x)
    new_y = int(y)
...
def move():
    ...
    """
    #if the robot is facing north increment y but must not over 5 after increment
...
def left():
    ...
    robot_location[2] = directions[current_face_index - 1] if current_face_index > 0 else directions[3]
...
def right():
    ...
    """
    current_face_index = directions.index(robot_location[2])
    robot_location[2] = directions[current_face_index + 1] if current_face_index < 3 else directions[0]
...
def execute_command(command):
    ...
    command --> one of the commands available (i.e. "PLACE", "MOVE", "LEFT", "RIGHT", "MOVE", "REPORT", or "STOP")
    """
